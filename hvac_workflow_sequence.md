...
### Enum sets are generated, not transcribed

*Added 2026-08-19 (v1.19.0), after hand-written SubType tables were found wrong in
four places.* `RULESET_ENUMS` in the editor is generated by
`tools/extract_ruleset_enums.js` and re-checked by `test_code_year.js`. Nothing about
which `Type` or `SubType` values a code year allows is written by hand.

Three rules the generator exists to enforce:

1. **Read each year from its own install.** The 2025 install also ships
   `T24N_2022 BEMEnums.txt`, but it is a backward-compatibility superset rather than
   the 2022 ruleset — it lists `ZnSys:EvaporativeCooler`, which the 2022 install's own
   enum does not. Generating 2022 from the bundled file silently breaks year gating.
   (2028 is the exception by necessity: it has no install of its own, so the bundled
   file is the only source, and system types simply aren't modelled for it yet.)
2. **Parse conditional branches; never union them.** Merging `When:` branches hid that
   `AirSys:Type` narrows to five types — SZAC, SZHP, SZDFHP, SPVAC, SPVHP — when
   `AirSys:ServesResZn = 1`. The branch is now in the tables. It is *not yet applied*:
   `ServesResZn` is derived by CBECC and appears in no project file, so honouring it
   means computing whether a system serves a residential zone. **Open item.**
3. **Resolve conditions by id, not by the file's own comments.** In the 2025
   `ZnSys:SubType` block, the condition on `Type` id 22 is commented `Baseboard` but 22
   is `PassiveBeam`, and a block commented `PassiveBeam` keys off id 23, which is not a
   `ZnSys:Type` at all.

What the hand-written tables had wrong, for the record:

| Type | Ruleset says | Had been |
|---|---|---|
| `ZnSys:SZDFHP` | else-branch, 13 values, default `NA` | its sibling SZHP's packaged/split set |
| `ZnSys:Baseboard` | else-branch, 13 values, default `NA` | no SubType at all |
| `MiniSplitAC` / `MiniSplitHP` | `Split3Phase` / `Split1Phase`, default `Split1Phase` | no SubType at all |
| `AirSys:HV`, `AirSys:Exhaust` | `NA` only, and a declared default the block never offers | no SubType (right answer, wrong reason) |

The one SubType decision the ruleset does not make is presentational: a Type whose only
legal value is `NA` gets no control, since there is no choice to make. That is recorded
as a UI rule in `hasSubType`, not as a claim about legality.
